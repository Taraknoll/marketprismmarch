...
1. ~~No timeout + fail-closed on `auth/v1/user` and `subscriptions` in `_require-auth.js`/`session.js` (locks out paying users).~~ **Addressed 2026-07-23:** 8s AbortControllers on both calls; provider-unavailable (timeout/5xx/429) now distinguished from definite rejection. Verify-unavailable → bounded 302 `/login?next=…&reason=auth_unavailable` (login page shows outage banner + retry link and suppresses its auto-redirect to avoid a bounce loop; jsonOnly gets 503). Subscription-unavailable for an already-verified user → **fail open** with `subscription.status='unknown_degraded'` (repair call skipped; 30s SUB_CACHE re-checks so the paywall reasserts once Supabase answers). `session.js` returns retryable 503 instead of 401 when Supabase can't answer. Client side: `ensureDashboardAuth` in `_template.html`/`_template_black.html` races `getSession()` against 8s and falls back to the persisted `sb-*-auth-token` localStorage session (expired tokens rejected → normal /login bounce).
