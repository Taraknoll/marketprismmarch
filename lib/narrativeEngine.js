...
function buildTldr(t, vk, family, fvd, energy, rbi, rbiZone) {
  ...
  const energyClause =
    energy == null ? '' :
    !energyIsReliable(energy) ? ` Narrative energy is recalibrating for ${t} — the decay engine hasn't locked a read yet.` :
...
function buildVerdictExplain(t, vk, energy, decay, coordination) {
  ...
  if (energy != null) {
    if (!energyIsReliable(energy)) parts.push(`Narrative energy is recalibrating for ${t} — the decay engine hasn't locked a read yet.`);
