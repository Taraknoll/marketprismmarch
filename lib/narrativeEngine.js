...
// The decay engine pins energy_remaining at exactly 100 when it can't fit a
// half-life curve for a narrative — confirmed across ~30% of narrative_scorecard
// rows sitewide, uniformly across every verdict (including bearish ones), and
// on narratives decades past any plausible half-life. That's a fallback
// artifact, not a real "fresh" reading, so treat it as no-data rather than
// asserting elevated/fresh energy. Real fitted values are essentially never
// an exact round 100.
const ENERGY_FALLBACK_THRESHOLD = 99.5;
function energyIsReliable(e) {
  return e != null && e < ENERGY_FALLBACK_THRESHOLD;
}
...
function buildTldr(t, vk, family, fvd, energy, rbi, rbiZone) {
  ...
  const energyClause =
    energy == null ? '' :
    !energyIsReliable(energy) ? ` Narrative energy has no reliable reading yet — decay history is still building for this story.` :
...
function buildVerdictExplain(t, vk, energy, decay, coordination) {
  ...
  if (energy != null) {
    if (!energyIsReliable(energy)) parts.push(`Narrative energy has no reliable reading yet for this story — decay history is still building.`);
    else if (energy > 70) parts.push(`Narrative energy remains elevated at ${energy.toFixed(0)}%, so the story still has momentum.`);
...
function buildMetaDescription(t, vk, family, fvd, energy) {
  ...
  if (energyIsReliable(energy)) parts.push(`Narrative energy ${energy.toFixed(0)}%.`);
