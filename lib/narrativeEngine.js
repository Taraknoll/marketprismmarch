...
const VERDICT_COPY = {
  ...
  monitoring: {
    ...
    outlook:  t => `${t} is at a decision point. The signals to watch: narrative energy direction, fair-value convergence, and any shift in institutional positioning.`,
  },
  supported: {
  ...
  neutral: {
    ...
    outlook:  t => `${t} is in a transitional phase. The signals to watch: narrative energy direction, fair-value convergence, and institutional positioning.`,
  },
};
...
function buildSummary(t, vk, family, fvd, energy, vms) {
  ...
  if (fvd != null && Math.abs(fvd) >= 1) {
    parts.push(`trading ${fmtPct(fvd)} ${fvd > 0 ? 'above' : 'below'} estimated fair value`);
  }
  if (energy != null) {
...
function buildOvervalued(t, fvd, vk, family, rbi, rbiZone) {
  // No fair-value baseline (e.g. recent IPOs): fall back to the Reality-Belief
  ...
  if (fvd == null) {
    if (rbi != null && rbiZone) {
      ...
      return `${t} doesn't yet have a stable fair-value baseline, so Market Prism reads valuation through narrative instead. ${zoneCopy[rbiZone] || `The Reality-Belief index sits at ${Math.round(rbi)}/100.`}`;
    }
    return `${t} doesn't yet have a stable fair-value baseline — common for newly public or thinly covered names. Watch the narrative and positioning signals below for valuation context until fundamentals settle.`;
  }

  const parts = [];
  if (fvd > 20) {
    parts.push(`${t} is trading ${fmtPct(fvd)} above its estimated fair value, a level that flags significant overvaluation risk.`);
  } else if (fvd > 0) {
    parts.push(`${t} is trading ${fmtPct(fvd)} above estimated fair value — a modest premium that may or may not be justified by growth expectations.`);
  } else if (fvd > -10) {
    parts.push(`${t} is trading near estimated fair value (${fmtPct(fvd)} deviation), suggesting balanced pricing.`);
  } else {
    parts.push(`${t} appears undervalued, trading ${fmtPct(fvd)} below estimated fair value.`);
  }
...
function buildMetaDescription(t, vk, family, fvd, energy) {
  ...
  if (fvd != null) parts.push(`${fvd > 0 ? '+' : ''}${fvd.toFixed(1)}% fair-value deviation.`);
